        ...
        const render = () => {
            ...
            starsRef.current.forEach((star) => {
                const opacity =
                    ...
                    0.7 *
                        Math.abs(
                            Math.sin(time * star.blinkSpeed + star.blinkOffset)
                        );
        ...
        const render = () => {
            ...
            for (let l = 0; l < waveLayers; l++) {
                ...
                for (let c = 0; c < columns; c++) {
                    ...
                    let y =
                        Math.sin(x * 0.005 + time * speed + layerSeed) *
                        amplitude;

                    // 2. 次波浪 (Medium Chop): 频率稍快，稍微反向或错位，打破规律
                    y +=
                        Math.sin(
                            x * 0.012 - time * speed * 0.5 + layerSeed * 2
                        ) *
                        (amplitude * 0.4);
                    ...
                    y +=
                        Math.sin(x * 0.027 + time * speed * 1.3) *
                    ...
                    // 像素量化
                    const quantizedY =
                        Math.floor((h - yOffset - y) / pixelSize) * pixelSize;
...
const PRESETS = [
    ...
    {
        value: "AS",
        label: "Alaska Airlines Mileage Plan",
        type: "airline",
        code: "AS",
...
export default function PointsTracker() {
    ...
    const totalValue = items.reduce(
        (acc, curr) => acc + ((curr.balance || 0) * (curr.cpp || 0)) / 100,
        0
    );
    ...
    return (
        <div
            className={`min-h-screen font-sans p-4 md:p-8 pb-32 relative overflow-hidden transition-colors duration-1000 ${theme.bg}`}
        >
            {/* --- 背景装饰 (Background Decoration) - 适配深色 --- */}
            <div className="fixed top-0 left-0 w-full h-full overflow-hidden -z-10 pointer-events-none">
                {/* 左上角 */}
                <div
                    className={`absolute top-[-10%] left-[-10%] w-[500px] h-[500px] rounded-full mix-blend-multiply filter blur-3xl animate-blob transition-colors duration-1000 ${
                        darkMode
                            ? "bg-indigo-900/40 opacity-40"
                            : "bg-indigo-100 opacity-70"
                    }`}
                ></div>
                {/* 右上角 */}
                <div
                    className={`absolute top-[-10%] right-[-10%] w-[500px] h-[500px] rounded-full mix-blend-multiply filter blur-3xl animate-blob animation-delay-2000 transition-colors duration-1000 ${
                        darkMode
                            ? "bg-blue-900/40 opacity-40"
                            : "bg-purple-100 opacity-70"
                    }`}
                ></div>
                {/* 左下角 */}
                <div
                    className={`absolute bottom-[-10%] left-[20%] w-[500px] h-[500px] rounded-full mix-blend-multiply filter blur-3xl animate-blob animation-delay-4000 transition-colors duration-1000 ${
                        darkMode
                            ? "bg-purple-900/40 opacity-40"
                            : "bg-pink-100 opacity-70"
                    }`}
                ></div>
                {/* 噪点纹理 */}
                <div
                    className={`absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] ${
                        darkMode ? "opacity-5" : "opacity-20"
                    }`}
                ></div>
            </div>
            ...
            <div className="max-w-4xl mx-auto relative z-10">
                ...
                <header className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-6">
                    <div className="flex-1">
                        <h1
                            className={`text-2xl font-bold flex items-center mb-2 ${theme.headerText}`}
                        >
                            <div
                                className={`p-2 backdrop-blur-sm rounded-xl mr-3 shadow-sm border ${theme.iconBg}`}
                            >
                        ...
                        <div className="flex gap-6 text-sm">
                            <div>
                                <p
                                    className={`${theme.subText} mb-0.5 font-medium`}
                                >
                                    积分总额
                                </p>
                                <p
                                    className={`text-2xl font-bold tracking-tight tabular-nums font-sans ${theme.headerText}`}
                                >
                            ...
                            >
                                <p
                                    className={`${theme.subText} mb-0.5 flex items-center font-medium`}
                                >
                                    估值 (CAD){" "}
                                    <TrendingUp
                                        size={14}
                                        className={`ml-1 ${theme.accentText}`}
                                    />
                                </p>
                                <p
                                    className={`text-2xl font-bold tracking-tight tabular-nums ${theme.accentText}`}
                                >
                                    ${" "}
                    ...
                    <div className="flex flex-col md:flex-row md:items-center gap-4 md:self-end">
                        {/* 筛选器 */}
                        <div
                            className={`backdrop-blur-md p-1 rounded-xl border flex text-sm shadow-sm ${
                                darkMode
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {filteredItems.map((item) => {
                        ...
                        return (
                            <div
                                ...
                                className={`group backdrop-blur-sm rounded-2xl border p-5 shadow-sm hover:shadow-xl transition-all duration-300 cursor-pointer relative overflow-hidden flex flex-col justify-between min-h-[150px] ${
                                    theme.cardBg
                                } ${
                                    darkMode
                                        ? "hover:bg-slate-800 hover:shadow-indigo-900/20 hover:border-indigo-500/30"
                                        : "hover:shadow-indigo-100/50 hover:-translate-y-1"
                                }`}
                            >
                                {/* 顶部颜色条 */}
                                <div
                                    className={`absolute top-2 left-2 bottom-2 w-1.5 rounded-full ${item.color} opacity-80`}
                                ></div>
                                ...
                                <div className="flex justify-between items-start pl-4">
                                    <div className="flex items-center space-x-3">
                                        ...
                                        <div>
                                            <h3
                                                className={`font-bold text-sm leading-tight ${theme.headerText}`}
                                            >
                                                {item.programName}
                                            </h3>
                                            <div
                                                className={`flex items-center text-xs font-medium mt-1.5 w-fit px-2 py-0.5 rounded-full ${
                                                    darkMode
                                    ...
                                    <div
                                        className={`opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-2 group-hover:translate-x-0 hover:text-indigo-500 ${
                                            darkMode
                                                ? "text-slate-600"
                                                : "text-slate-300"
                                        }`}
                                ...
                                {/* 数据区 */}
                                <div
                                    className={`pl-4 pt-4 mt-2 border-t flex justify-between items-end ${
                                        darkMode
                                            ? "border-slate-800"
                                            : "border-slate-100/50"
                                    }`}
                                >
                                    <div>
                                        <div
                                            className={`text-2xl font-bold tracking-tight leading-none tabular-nums font-sans ${theme.headerText}`}
                                        >
                                            {item.balance.toLocaleString()}
                                        </div>
                                        <div
                                            className={`text-xs mt-1.5 font-medium ml-0.5 tabular-nums ${theme.subText}`}
                                        >
                                            @{item.cpp} cpp
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <div
                                            className={`text-sm font-bold px-2.5 py-1.5 rounded-lg border tabular-nums ${
                                                isBank
                                                    ? darkMode
                    ...
                    <button
                        onClick={() => openModal()}
                        className={`group border-2 border-dashed rounded-2xl p-4 flex flex-col items-center justify-center transition-all duration-300 min-h-[150px] ${
                            darkMode
                                ? "border-slate-800 bg-slate-900/30 text-slate-500 hover:text-indigo-400 hover:border-indigo-500/50 hover:bg-slate-800"
                                : "border-slate-300/60 bg-white/30 text-slate-400 hover:border-indigo-400 hover:text-indigo-600 hover:bg-indigo-50/50"
                        }`}
                    >
                        <div
                            className={`p-3 rounded-full shadow-sm mb-3 group-hover:scale-110 transition-transform duration-300 ${
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-all">
                    <div
                        className={`backdrop-blur-xl rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in duration-200 border ${theme.modalBg}`}
                    >
                        <div
                        ...
                        >
                            <h3
                                className={`font-bold text-lg ${theme.headerText}`}
                            >
                        ...
                        <form onSubmit={handleSave} className="p-6 space-y-5">
                            ...
                            <div>
                                <label
                                    className={`block text-xs font-bold uppercase tracking-wider mb-1.5 ${theme.subText}`}
                                >
                                    选择积分项目
                                </label>
                            ...
                            {formData.isCustom && (
                                <div
                                    className={`space-y-4 p-4 rounded-xl border animate-in fade-in slide-in-from-top-2 ${
                                        darkMode
                            ...
                            {!formData.isCustom && (
                                ...
                                >
                                    <div
                                        className={`w-12 h-12 rounded-xl flex items-center justify-center text-white font-bold text-lg shadow-md ${formData.color}`}
                                    >
                                        {formData.programCode}
                                    </div>
                                    <div>
                                        <p
                                            className={`font-bold ${theme.headerText}`}
                                        >
                                            {formData.programName}
                                        </p>
                                        <p
                                            className={`text-xs font-medium mt-0.5 ${theme.subText}`}
                                        >
                                            预置项目
                                        </p>
                                    </div>
                            ...
                            <div className="grid grid-cols-2 gap-5 pt-1">
                                ...
                                <div>
                                    <label
                                        className={`flex justify-between text-xs font-bold uppercase tracking-wider mb-1.5 ${theme.subText}`}
                                    >
                                        <span>估值 (CPP)</span>
                                    </label>
                                    <div className="relative">
                            ...
                            <div
                                className={`pt-4 flex gap-3 border-t mt-2 ${
                                    darkMode
                                        ? "border-slate-800"
                                        : "border-slate-100"
                                }`}
                            >
                                ...
                                <button
                                    type="submit"
                                    className={`flex-1 bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-3 rounded-xl text-sm font-bold transition-all shadow-lg shadow-indigo-500/20 active:scale-95 ${
                                        !editingItem ? "w-full" : ""
                                    }`}
                                >
                                    保存更改
